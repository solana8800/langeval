...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!resourceResp.ok) {
      ...
      console.error("Resource Service Error Body:", errText);
    ...
    const orchestratorUrl = process.env.ORCHESTRATOR_SERVICE_URL || "http://localhost:8001";
    const orchestratorResp = await fetch(`${orchestratorUrl}/orchestrator/red-teaming`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        scenario_id: "red-teaming-generic", // Placeholder hoặc ID thực tế
        agent_id: agentId,
        language: language || "en",
        metadata: {
    ...
    if (!orchestratorResp.ok) {
      ...
      console.error("Orchestrator Error Body:", errText);
