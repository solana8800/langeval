...
export async function GET(request: Request) {
  ...
  try {
    ...
    const campaignResp = await fetch(`${RESOURCE_SERVICE_URL}/resource/battle/campaigns/${campaignId}`);
    if (!campaignResp.ok) throw new Error('Campaign not found');
    const campaign = await campaignResp.json();

    // 2. Lấy danh sách Turns
    const turnsResp = await fetch(`${RESOURCE_SERVICE_URL}/resource/battle/campaigns/${campaignId}/turns`);
