...
export default function ScenarioListPage() {
   ...
   // Fetch models
   useEffect(() => {
      fetch('/api/v1/resource/models?page_size=100')
         .then(res => res.json())
         .then(data => {
   ...
   const { data: apiResponse, error, isLoading, mutate } = useSWR(
      `/api/v1/resource/scenarios?agent_id=${selectedAgent}&search=${searchQuery}&page_size=100`,
      fetcher
   );
   ...
   const handleCreateScenario = async () => {
      ...
      try {
         ...
         const res = await fetch('/api/v1/resource/scenarios', {
            method: 'POST',
   ...
   const confirmDelete = async () => {
      ...
      try {
         await fetch(`/api/v1/resource/scenarios/${itemToDelete}`, { method: 'DELETE' });
         mutate();
      } catch (e) {
   ...
   const handleUpdateScenario = async () => {
      setIsEditing(true);
      try {
         const res = await fetch(`/api/v1/resource/scenarios/${editingScenarioId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
