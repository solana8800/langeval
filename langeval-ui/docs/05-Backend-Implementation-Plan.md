...
| :--- | :--- | :--- | :--- |
| **Identity Service** | Python (FastAPI) + Entra External ID | User Management, JWT Auth, RBAC, Auto-provisioning. | `settings/team`, `profile` |
...
| **Redis** | Infrastructure (Docker) | Caching, LangGraph Checkpointer (Persistence). | N/A (Backend Internal) |
...
Lưu trữ thông tin chi tiết về các target chatbot cần được đánh giá.
...
async def list_agents(
    ...
    """Lấy danh sách Agent hỗ trợ phân trang"""
    return agent_service.get_multi(db, page=page, size=size)

@router.post("/agents", response_model=AgentRef)
async def create_agent(config: AgentCreate, db: Session = Depends(get_db)):
    """Tạo Agent và mã hóa API Key"""
    return agent_service.create(db, obj_in=config)
```
...
### A. Auth & Users (`/api/v1/identity`)
- [x] `GET /health`: Health check.
- [x] `GET /me`: User Profile (Synced from Entra).
- [ ] `POST /team/invite`: Mời thành viên mới (Planned).
- [ ] `PATCH /team/role`: Cập nhật Role (Planned).

### B. Agents Management (`/api/v1/resource/agents`)
- [x] `POST /resource/agents`: Create new agent config (Encrypted).
- [x] `GET /resource/agents`: List monitored agents (Pagination).
- [x] `PUT /resource/agents/{id}`: Update agent config.

### C. Evaluation Campaigns (`/api/v1/orchestrator/campaigns`)
