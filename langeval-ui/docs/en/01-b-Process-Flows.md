...
Automated workflow for generating a "Golden Dataset" from raw business documents.
...
Core workflow for running a Test Campaign.

### 2.1. Orchestration Flow (LangGraph)
State machine of a single test run.
...
Ensuring evaluation quality when the AI is uncertain.
...
Blind comparison workflow between Model A and Model B.
...
Handling failures during long-running tests.
