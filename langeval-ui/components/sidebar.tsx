...
export function Sidebar() {
  ...
  const navContent = (
    <div className="flex h-full flex-col overflow-hidden bg-white">
      {/* Header */}
      {/* Header */}
      <div className={cn("flex h-[60px] items-center px-6 bg-white shrink-0 transition-all", isCollapsed ? "justify-center px-2" : "border-b")}>
      ...
      <div className={cn("border-t bg-slate-50/50 shrink-0 flex flex-col gap-1", isCollapsed ? "p-2 items-center" : "p-3")}>

        {/* Desktop Language Switcher */}
        {!isCollapsed && (
          <div className="flex items-center justify-between mb-2 px-2">
            <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">{t("languages")}</span>
            <div className="flex items-center gap-1.5 bg-white rounded-full px-2 py-0.5 border shadow-sm">
              <button
                onClick={() => router.replace(pathname, { locale: 'en' })}
                className={cn("text-[10px] font-bold px-1 rounded transition-colors", locale === 'en' ? "text-indigo-600 bg-indigo-50" : "text-slate-400 hover:text-slate-600")}
              >
                EN
              </button>
              ...
              <button
                ...
                className={cn("text-[10px] font-bold px-1 rounded transition-colors", locale === 'vi' ? "text-indigo-600 bg-indigo-50" : "text-slate-400 hover:text-slate-600")}
              >
                VI
              </button>
            </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <div className={cn(
              "flex items-center gap-2 cursor-pointer hover:bg-slate-100 rounded-md transition-colors w-full",
              isCollapsed ? "justify-center p-2" : "p-2"
            )}>
              <Avatar className="h-8 w-8 border shrink-0">
                <AvatarImage src="/avatars/01.png" alt="@vf-admin" />
                <AvatarFallback className="bg-primary text-primary-foreground text-xs">LE</AvatarFallback>
              </Avatar>

              {!isCollapsed && (
                <>
                  <div className="flex flex-col flex-1 min-w-0 animate-in fade-in">
                    <p className="text-xs font-semibold text-slate-900 truncate">{t("adminUser")}</p>
                    <p className="text-[10px] text-muted-foreground truncate">admin@langeval.ai</p>
                  </div>
                  <Settings className="h-5 w-5 text-slate-400 hover:text-slate-900" />
                </>
              )}
            </div>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="w-56" align="end" forceMount side={isCollapsed ? "right" : "top"}>
            <DropdownMenuLabel className="font-normal">
              <div className="flex flex-col space-y-1">
                ...
                <p className="text-xs leading-none text-muted-foreground">admin@langeval.ai</p>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuGroup>
              ...
              <DropdownMenuItem asChild>
                <I18nLink href="/settings/team" className="cursor-pointer">
      ...
      <div className="lg:hidden fixed top-4 left-4 z-50">
        <Sheet open={open} onOpenChange={setOpen}>
          ...
          <SheetContent side="left" className="p-0 w-72 border-r-0 gap-0">
            ...
            <div className="flex h-full flex-col overflow-hidden bg-white">
              <div className="flex h-[60px] items-center border-b px-6 bg-white shrink-0">
                <I18nLink className="flex items-center gap-2 font-bold text-xl text-primary" href="/" onClick={() => setOpen(false)}>
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-600 to-violet-600 flex items-center justify-center shadow-lg shadow-indigo-500/20">
                    <span className="font-bold text-white text-lg">E</span>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto px-3 py-4 flex flex-col">
                ...
                <div className="mt-auto pt-6 px-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2 bg-slate-50 rounded-full px-2 py-0.5 border">
                      <button
                        onClick={() => router.replace(pathname, { locale: 'en' })}
                        className={cn("text-[10px] font-bold px-1 rounded transition-colors", locale === 'en' ? "text-indigo-600 bg-indigo-50" : "text-slate-400 hover:text-slate-600")}
                      >
              ...
              <div className="border-t bg-slate-50/50 shrink-0 flex flex-col gap-1 p-3">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <div className="flex items-center gap-2 cursor-pointer hover:bg-slate-100 rounded-md transition-colors w-full p-2">
                      <Avatar className="h-8 w-8 border shrink-0">
                        <AvatarImage src="/avatars/01.png" alt="@vf-admin" />
                        <AvatarFallback className="bg-primary text-primary-foreground text-xs">LE</AvatarFallback>
                      </Avatar>
