...
export function ScenarioBuilder({ scenarioId, isSidebarOpen, setSidebarOpen }: ScenarioBuilderProps) {
  ...
  const [scenarioMeta, setScenarioMeta] = useState<{ name: string, description: string, agentId: string, agentName?: string, modelId?: string } | null>(null);
  ...
  const loadScenario = useCallback(() => {
    setLoading(true);
    fetch(`/api/v1/resource/scenarios/${scenarioId}`, { cache: 'no-store' })
      .then(res => {
        if (!res.ok) {
          console.warn(`Backend/API unreachable (${res.status}). Checking for local demo data...`);
          ...
          if (data.name || data.agent_id) {
            ...
            if (!agentName && data.agent_id) {
              ...
              if (found) {
                ...
                try {
                  fetch(`/api/v1/resource/agents/${data.agent_id}`)
                    .then(res => res.json())
                    .then(agentData => {
            ...
            setScenarioMeta({
              ...
              agentName: agentName,
              modelId: data.meta_data?.model_id // Load model_id from meta_data
            });
          }
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/v1/resource/scenarios/generate-ai', {
        method: 'POST',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/resource/scenarios/${scenarioId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleRun = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/orchestrator/campaigns`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const pollInterval = setInterval(async () => {
        try {
          const stateRes = await fetch(`/api/v1/orchestrator/campaigns/${campaignId}/state`);
          ...
          <div className="space-y-6 py-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(val) => setScenarioMeta(prev => prev ? { ...prev, modelId: val === "default_model" ? "" : val } : null)}
              >
