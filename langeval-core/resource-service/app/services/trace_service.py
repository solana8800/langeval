...
class TraceService:
    ...
    def _get_agent_and_client(self, agent_id: Optional[str] = None) -> tuple[Optional[str], Any]:
        """Returns Project ID and configured LangfuseClient"""
        if agent_id:
            agent = self.session.get(AgentRef, agent_id)
            if not agent or not agent.langfuse_project_id:
                return None, get_langfuse_client()
            
            # Use specific agent credentials if available
            host = agent.langfuse_host
            public_key = agent.langfuse_public_key
            secret_key = decrypt_value(agent.langfuse_secret_key_encrypted) if agent.langfuse_secret_key_encrypted else None
            
            # Get the client instance specialized for this agent
            client = get_langfuse_client(host=host, public_key=public_key, secret_key=secret_key)
            return agent.langfuse_project_id, client
        else:
            ...
            # If agent_id not specified, but we need "some" trace, we could use the first one 
            # (though technically trace fetch is usually scoped to specific agent in UI)
            agent = agents[0]
            host = agent.langfuse_host
            public_key = agent.langfuse_public_key
            secret_key = decrypt_value(agent.langfuse_secret_key_encrypted) if agent.langfuse_secret_key_encrypted else None
    ...
    async def get_traces(self, 
        ...
        project_id, client = self._get_agent_and_client(agent_id)
        if not project_id:
             return {
                "data": [],
                "total": 0,
    ...
    async def get_trace_detail(self, trace_id: str, agent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        result = await client.get_trace_detail(
            project_id=project_id,
            trace_id=trace_id
        )
