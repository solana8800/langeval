...
def create_kb(kb: KnowledgeBaseRef, session: Session = Depends(get_session)):
    service = KBService(session)
    return service.create_kb(kb)

@router.get("", response_model=Page[KnowledgeBaseRef])
def read_kbs(page: int = 1, page_size: int = 10, session: Session = Depends(get_session)):
    service = KBService(session)
    return service.get_kbs(page=page, page_size=page_size)

@router.put("/{id}", response_model=KnowledgeBaseRef)
def update_kb(id: str, kb_update: KnowledgeBaseUpdate, session: Session = Depends(get_session)):
    service = KBService(session)
    try:
...
@router.delete("/{id}")
def delete_kb(id: str, session: Session = Depends(get_session)):
    service = KBService(session)
    try:
