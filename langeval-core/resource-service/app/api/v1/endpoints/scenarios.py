...
def create_scenario(scenario_in: ScenarioCreate, session: Session = Depends(get_session)):
...
def read_scenario(id: str, session: Session = Depends(get_session)):
    service = ScenarioService(session)
...
def update_scenario(id: str, scenario_update: ScenarioUpdate, session: Session = Depends(get_session)):
    service = ScenarioService(session)
    try:
...
@router.delete("/{id}")
def delete_scenario(id: str, session: Session = Depends(get_session)):
    service = ScenarioService(session)
