...
class LangfuseClient:
    ...
    def __init__(self, host: Optional[str] = None, public_key: Optional[str] = None, secret_key: Optional[str] = None):
        self.base_url = host or os.getenv("LANGFUSE_HOST", "http://localhost:3000")
...
# Dictionary to store singleton instances per agent configuration
_langfuse_clients: Dict[str, LangfuseClient] = {}

def get_langfuse_client(host: Optional[str] = None, public_key: Optional[str] = None, secret_key: Optional[str] = None) -> LangfuseClient:
    """Trả về singleton instance của LangfuseClient dựa trên cấu hình (hoặc mặc định)"""
    global _langfuse_clients
    
    # Create a unique key based on the configuration
    cache_key = f"{host or 'default'}_{public_key or 'default'}"
    
    if cache_key not in _langfuse_clients:
        _langfuse_clients[cache_key] = LangfuseClient(host=host, public_key=public_key, secret_key=secret_key)
        
    return _langfuse_clients[cache_key]
